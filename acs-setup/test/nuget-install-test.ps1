# nuget-install-test.ps1 - Simple NuGet installation test

Write-Host "=== NuGet Installation Test ===" -ForegroundColor Green

# Check if NuGet already exists
Write-Host "Checking for existing NuGet..." -ForegroundColor Yellow
$nuget = Get-Command "nuget.exe" -ErrorAction SilentlyContinue
if ($nuget) {
    Write-Host "NuGet already installed at: $($nuget.Source)" -ForegroundColor Green
    exit 0
}

Write-Host "NuGet not found. Starting installation..." -ForegroundColor Yellow
...
Write-Host "`nInstallation complete! You can now run your main script." -ForegroundColor Green